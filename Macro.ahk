...
InitializeMacro() {
    ...
    if (ok:=FindText(&X, &Y, 746, 476, 862, 569, 0, 0, AreasText)) {
        ...
        MsgBox("You must be in the lobby to start the macro, go through #common-issues in the discord to fix this if it isnt working..", "Error T3", "+0x1000", )
        return
    }
        ...
        while (placementCount < placements && y >= endY) { ; Rows
            while (placementCount < placements && x <= endX) { ; Columns
                ...
                if ShouldStopUpgrading(1) {
                    AddToLog("Stopping due to finding return to lobby.")
                    return LobbyLoop()
                }
                Reconnect()
